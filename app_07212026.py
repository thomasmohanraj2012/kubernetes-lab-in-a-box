...
with col4:
    st.metric("🚀 Deployments", deployments)

if k8s_connected:
    st.success("✅ Connected to Minikube")
else:
    st.error("❌ Kubernetes Connection Failed")
...
if k8s_connected:
    ...
    node_info = v1.list_node().items[0]

    st.subheader("☸️ Cluster Health")

    st.info(f"""
Node Name: {node_info.metadata.name}

Status: Ready ✅

Kubernetes Version:
{node_info.status.node_info.kubelet_version}
""")
    ...
    if st.button(
        ...
        try:
            ...
            for pod in pods_list.items:
                ...
                if pod.metadata.name == "nginx-lab":
            ...
            if pod_found:
                ...
                st.success(
                    f"✅ Lab Passed! Found pod: {found_pod_name}"
                )
...
""")
                ...
                st.error(
                    "❌ Lab Failed! nginx-lab pod not found."
                )

                st.warning("""
Create the lab pod first:

kubectl run nginx-lab --image=nginx
""")
